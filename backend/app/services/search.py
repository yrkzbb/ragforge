...
class SearchService:
    ...
    async def search(self,knowledge_base_id,query:str,top_k:int=10,retrieve_k:int=30):
        ...
        rows=(await self.db.execute(select(Chunk,Parent.text).join(Document,Chunk.document_id==Document.id).outerjoin(Parent,Chunk.parent_id==Parent.id).where(Document.knowledge_base_id==knowledge_base_id,Chunk.level=="child"))).all()
        candidates=[Candidate(str(c.id),c.text,c.breadcrumb,parent or c.text) for c,parent in rows]
        ...
        with tracer.start_as_current_span("dense_retrieval"):
            ...
            try:
                ...
                result=(await self.db.execute(select(Chunk,Parent.text,(1-Chunk.embedding.cosine_distance(vector)).label("score")).join(Document,Chunk.document_id==Document.id).outerjoin(Parent,Chunk.parent_id==Parent.id).where(Document.knowledge_base_id==knowledge_base_id,Chunk.level=="child",Chunk.embedding.is_not(None)).order_by(Chunk.embedding.cosine_distance(vector)).limit(retrieve_k))).all()
                dense=[Candidate(str(c.id),c.text,c.breadcrumb,parent or c.text,dense_score=float(score)) for c,parent,score in result]
